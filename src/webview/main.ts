...
try {
  ...
  function renderFileChangesSummary(payloadText: string): string {
    try {
    ...
    if (streamRaw.length === 0) {
      ...
      if (hasAction) {
        ...
        if (streamRaw.includes('<edit_file')) {
          ...
          const server = streamRaw.match(/server="([^"]+)"/);
          const tool = streamRaw.match(/tool="([^"]+)"/);
          actionPath = [server?.[1], tool?.[1]].filter(Boolean).join('.');
        }
