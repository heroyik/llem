...
export function loadMcpServers(options: McpConfigLoadOptions = {}): McpConfigLoadResult {
    ...
    pushConfig('llem:user', 55, readLlemMcpServers(path.join(homeDir, '.llem', 'mcp.json'), env, warnings));
    ...
    if (sources.has('workspace') && options.workspaceRoot) {
        ...
        pushConfig('workspace:.llem/mcp.json', 60, readLlemMcpServers(path.join(options.workspaceRoot, '.llem', 'mcp.json'), env, warnings));
    }
...
function readLlemMcpServers(filePath: string, env: NodeJS.ProcessEnv, warnings: string[]): McpServersConfig {
    const parsed = readJsonFile(filePath, warnings);
    if (!parsed) {
        return {};
    }
    return isRecord(parsed.mcpServers)
