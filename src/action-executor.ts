...
import type { SafePathResult } from './security';
...
/** Re-export the original name for backward compat (used in actionReport.ts) */
export type ActionExecutionHost = ActionExecutorDeps;
...
export function createActionExecutor(deps: ActionExecutorDeps): ActionExecutor {
    ...
    async function executeActions(aiMessage: string): Promise<string[]> {
        const traceId = buildActionTraceId();
        const executionMode = deps.getExecutionMode?.() ?? 'default';
        ...
        if (ctx.fileResult.chatMessage) {
            deps.appendChatMessage(ctx.fileResult.chatMessage);
        }
